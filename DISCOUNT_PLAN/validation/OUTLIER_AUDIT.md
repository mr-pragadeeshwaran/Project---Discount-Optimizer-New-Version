...
*Run `20260711_221318` · 3129 outlier days removed by the |z|>2.0 filter, cross-checked against the festival calendar, platform-event windows, stock-outs, and each cell's own promo depth. ADVISORY: this audits the filter, it does not change it — the champion's training data stays exactly as validated.*
...
| Deep promo (HIGH, ≥5ppt above cell's median discount) | 351 | 11% | A documented promo did what promos do — right to exclude from *regular-day* training |
| Festival window (±2d) | 0 | 0% | Calendar-driven demand |
| Platform event (BBD etc.) | 0 | 0% | Platform-driven demand |
| **Unexplained** | 2778 | 89% | Statistical noise or undocumented events |
...
- Of the regular days the filter removed, 351 (11%) coincide with the cell's own deep-discount days — documented promo behavior, correctly kept out of *regular-day* training.
- The 2778 unexplained days (89%) are the statistical tail the filter exists to remove: 3129 removals = 2.8% of all rows, consistent with a |z|>2 cut, not with silently eaten demand signal. Some may be promos nobody logged — if a big cut decision ever hinges on one cell, check its unexplained outliers in `outlier_promo_audit.csv` first.
