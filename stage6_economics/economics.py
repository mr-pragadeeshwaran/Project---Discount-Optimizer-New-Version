...
def compute_economics(curves_df: pd.DataFrame,
    ...
    for _, cell in curves_df.iterrows():
        ...
        # Get cost parameters (from master or defaults)
        costs = _get_costs(cell["product_id"], mrp, master_costs)

        # Evaluate economics at each discount level
        ladder = []
        for pt in points:
            d = pt["discount_pct"]
            price = pt["price"]
        ...
        # Compute marginal ROI between adjacent discount levels
        ...
        # Find elbow: where marginal_roi crosses threshold (from above to below)
        elbow = _find_elbow(ladder_df, cfg.MARGINAL_ROI_THRESHOLD)

        # Compute changes: current → elbow
        ...
        results.append({
            ...
            "current_price": round(mrp * (1 - current_disc / 100), 1),
            ...
            "elbow_price": round(mrp * (1 - elbow["discount_pct"] / 100), 1),
            ...
            # Full ladder (for drill-down chart)
            "ladder": ladder,
